...
class Crawler:
    ...
    def request(self, request, parameters=None):
        try:
            if parameters is None:
                ...
                url = 'https://api.github.com/' + request + '?per_page=100&client_id=' + \
                    self.id + '&client_secret=' + self.secret
            else:
                print(('Processing request: ' + request + ' ' + str(parameters)))
                url = 'https://api.github.com/' + request + '?per_page=100&client_id=' + self.id + \
    ...
    def request2(self, request, parameters=None):
        ...
        try:
            ...
            if(range_pages == 1 ):
                ...
                #join requests list
                if (range_pages <= 100):
                    responses = list(chain.from_iterable(results_list[i] for i in range(len(results_list))))
                else:
                    aux1 = list(chain.from_iterable(results_list[i] for i in range(len(results_list))))
                    responses = list(chain.from_iterable(aux1[i] for i in range(len(aux1))))
    ...
    def link_generator(self,url,finalRange):
        link_list = []
        for page_number in range(finalRange):
    ...
    def range_max_verify(self, request,parameters=None):
        try:
            ...
            if(parameters!=None):
                url = url+"&"+"&".join(parameters)
            if 'stargazer' in url:
                request_header = urllib.request.Request(url)
                request_header.add_header('Accept','application/vnd.github.v3.star+json')
                response = urllib.request.urlopen(request_header)
            else:
    ...
    def verify_rate_limit(self, header):
        try:
            self.rate_limit_remaining = header["X-RateLimit-Remaining"]
            self.rate_limit_reset = header["X-ratelimit-reset"]
        except:
        ...
        if int(self.rate_limit_remaining) <= 50:
    ...
    def wait_internet_connection(self, request, parameters):
        ...
        while number_of_attempts < 10:
            try:
                response = urllib.request.urlopen('https://github.com', timeout=1)
                if response:
